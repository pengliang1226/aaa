...
def load_data(file_path='./model_data.fea', label='flag_y'):
    """
...
def run(kf, data, model, label='flag_y'):
    ...
    for i, index in enumerate(kf_list):
        ...
        model.fit(X_train, y_train, estimator__early_stopping_rounds=50, estimator__eval_set=[(X_test, y_test)], estimator__eval_metric='auc')
